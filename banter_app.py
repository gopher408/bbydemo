	    ...
	    print "prev_data: " + str(prev_data)
 	    ...
 	    if prev_data: 
               if 'datastore_locations' in prev_data and topic == 'location':
 	              ...
 	              if 'location' in resultData or 'facility' in resultData or 'department' in resultData:
                          ...
                          for num in range(0,total):
                              if resultData[curr_name].lower() in prev_data['datastore_locations'][num]['name'].lower():
                                 if 'location' in resultData:
                                 ...
                                 resultData['datastore_locations'][0] = prev_data['datastore_locations'][num]
                          if resultData['datastore_locations'][0]:
                             print "\n***** RESULT *****\n"
                             self.set_query(resultData)
                             print self.get_query()
                             return self.get_query()

            print "\nSome resultData: " + str(resultData)
	    	...
	    	if 'action' in newdata and newdata['action'] == 'ask time':
                    ...
                    if 'descriptor' not in resultData:
...
#		print "dayword: " + str(dayword)
                ...
                if dayword == 'monday':
                    ...
                    dayhours = data['datastore_location']['hours']['thu']
                    if datetimefield != 'today' and datetimefield != 'tomorrow':
                        data['datetime'] = dayword.title()
                elif dayword == 'friday':
		   ...
		   datetimefield = datetimefield.capitalize()
...
#		print "datetimefield: " + datetimefield
		...
		if dayhours != '':
                   ...
                   if num == 0:
                      ...
                      txt = ''
                   if ('descriptor' in data and 'how late' in data['descriptor']) or ('descriptor' in data and 'open' in data['descriptor'] and  'until' in data['descriptor']):
                      # Nordstrom Stanford Shopping Center opens until 9:00 PM tonight.
                      parts = dayhours.split('-')
                      self.set_response_text(data, data['datastore_location']['name'] + 
                                                ' is open until ' + parts[1] + ' ' + (
                        			txt + datetimefield if datetimefield else 'tonight') + '.')
                      ...
                      self.set_response_text(data, data['datastore_location']['name'] +
                                                ' opens at ' + parts[0] + ' ' + (
                                                txt + datetimefield if datetimefield else 'today') + '.')

                   elif dayword in ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']:
		      dayhours = dayhours.capitalize()
                      parts = dayhours.split('-')
                      self.set_response_text(data, data['datastore_location']['name'] + 
                                                ' is open from ' + parts[0] + ' until ' + parts[1] + ' ' + (
                                                'on ' + datetimefield if datetimefield else 'tonight') + '.')
		    ...
		    if not found:
                        self.set_response_text(data, 'We do not have your selection in stock. Would you like to look other products?')
...
if __name__ == '__main__':
    ...
    name_1 = "Banter"
    agent = BanterClient(name_1, banter_config, Echo(), realDataStore if use_datastore else dummyDataStore)

    ##### configure banter client for a customer
    name_2 = "Joe"
    customer = BanterClient(name_2, banter_config, Echo(), None)
